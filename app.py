...
OPTIONS = {
    "who_stage": [1, 2, 3, 4],
    "suppressed_lt1000": [0, 1],  # UI note: 0=No, 1=Yes
...
with st.expander("Debug: Check models folder"):
    ...
    if MODELS_DIR.exists():
        st.write("Files:", sorted([p.name for p in MODELS_DIR.iterdir()]))
    else:
        st.error("models/ folder not found beside app.py")

# -------------------------
# Load available models (pairs of .keras + _metadata.json)
# -------------------------
...
def detect_max_year_from_cols(cols) -> int:
    """
    Detect the highest _Y{n} suffix present in the dataframe columns.
    If only _Y1 exists => max_year=1 (baseline/Y1 info only) => use FirstVisit_to_Y1
    """
    max_year = 0
    for y in [1, 2, 3, 4, 5]:
...
def choose_model_key_by_year(max_year: int, keys: set):
    """
    Correct mapping:
      max_year == 1 -> DeepANN_FirstVisit_to_Y1
      max_year == 2 -> DeepANN_Y1_to_Y2
      max_year == 3 -> DeepANN_Y1Y2_to_Y3
      max_year == 4 -> DeepANN_Y1Y2Y3_to_Y4
      max_year >= 5 -> DeepANN_Y1Y2Y3Y4_to_Y5 (if present)
    """
    if max_year >= 5 and "DeepANN_Y1Y2Y3Y4_to_Y5" in keys:
        return "DeepANN_Y1Y2Y3Y4_to_Y5"
    if max_year >= 4 and "DeepANN_Y1Y2Y3_to_Y4" in keys:
        return "DeepANN_Y1Y2Y3_to_Y4"
    if max_year >= 3 and "DeepANN_Y1Y2_to_Y3" in keys:
        return "DeepANN_Y1Y2_to_Y3"
    if max_year >= 2 and "DeepANN_Y1_to_Y2" in keys:
        return "DeepANN_Y1_to_Y2"
    if max_year >= 1 and "DeepANN_FirstVisit_to_Y1" in keys:
        return "DeepANN_FirstVisit_to_Y1"
    return None
...
def predict_df(df_in: pd.DataFrame, model_info: dict):
    ...
    for k in ["feature_cols", "cat_cols", "num_cols"]:
        if k not in meta:
            raise KeyError(f"Metadata missing required key: '{k}'")
...
with tab1:
    ...
    st.markdown(
        "<div class='small-note'>Select the maximum year of data you have. The app will pick the right model and use the BestF1 threshold from its metadata.</div>",
        unsafe_allow_html=True,
    )

    # IMPORTANT: max_year=1 means baseline/Y1 info only => predicts Year 1
    ...
    schema_cols = set(meta.get("feature_cols", []))
    ...
    with st.container():
        ...
        for y in range(1, max_year + 1):
            with st.expander(f"Year {y} inputs", expanded=(y == 1)):
                ...
                with n1:
                    age = st.number_input(f"age_Y{y}", min_value=0, max_value=120, value=35, step=1, key=f"sp_age_{y}")
                    add_if_in_schema(row, f"age_Y{y}", float(age), schema_cols)

                with n2:
                    cd4 = st.number_input(f"cd4_Y{y}", min_value=0, max_value=5000, value=350, step=10, key=f"sp_cd4_{y}")
                    add_if_in_schema(row, f"cd4_Y{y}", float(cd4), schema_cols)

                with n3:
                    vl = st.number_input(f"viral_load_Y{y}", min_value=0.0, value=1000.0, step=50.0, key=f"sp_vl_{y}")
                    add_if_in_schema(row, f"viral_load_Y{y}", float(vl), schema_cols)

                n4, n5, n6 = st.columns(3)
                with n4:
                    logvl = st.number_input(f"log10_vl_Y{y}", min_value=0.0, value=3.0, step=0.1, key=f"sp_logvl_{y}")
                    add_if_in_schema(row, f"log10_vl_Y{y}", float(logvl), schema_cols)

                with n5:
                    wt = st.number_input(f"weight_Y{y}", min_value=0.0, value=60.0, step=0.5, key=f"sp_wt_{y}")
                ...
                with n8:
                    missed = st.number_input(f"missed_appointments_Y{y}", min_value=0, value=0, step=1, key=f"sp_missed_{y}")
                    add_if_in_schema(row, f"missed_appointments_Y{y}", float(missed), schema_cols)

                with n9:
                    late = st.number_input(f"days_late_Y{y}", min_value=0, value=0, step=1, key=f"sp_late_{y}")
                    add_if_in_schema(row, f"days_late_Y{y}", float(late), schema_cols)

                if y == 1:
                    base_age = st.number_input("age_baseline_Y1", min_value=0, max_value=120, value=35, step=1, key="sp_base_age")
